#include "roadimg.h"  //your project's name
#include "ui_roadimg.h"//your project's name
...
int OTSU(const Mat gImage);

void roadimg::openImage()
{
	QString pFileName; 
	pFileName=QFileDialog::getOpenFileName(0,QString::fromLocal8Bit("Please Select The IMG"),QString::fromLocal8Bit("C:\\Users\\zj790\\Desktop\\"),/*".",*/QString::fromLocal8Bit("Images (*.png *.bmp *.jpg *.tif *.GIF )"));
		QPixmap image(pFileName);
...
int OTSU(Mat gImage) 
{
	int nCols = gImage.cols;
	int nRows = gImage.rows;
	int threShold = 0;
	//init the parameters
	int nSumPix[256];
	float nProDis[256];
	...
	for (int i = 0; i < nRows; i++)
	{
		...
		{
			nSumPix[(int)gImage.at<uchar>(i, j)]++;
		}
	}
